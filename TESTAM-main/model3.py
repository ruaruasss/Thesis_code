...
class TESTAM(nn.Module):
    ...
    TESTAM model
    """
    def __init__(self, num_nodes, dropout=0.3, in_dim=2, out_dim = 1, hidden_size = 32, layers = 3, prob_mul = False, max_time_index = 288, mlp_hidden_dim=256, **args):
        ...
        # Define the MLP for processing combined support matrices
        self.gating_mlp = nn.Sequential(
            #nn.Linear(3 * num_nodes * num_nodes, mlp_hidden_dim),  # 输入维度3*N²
            nn.Linear(num_nodes * num_nodes, mlp_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(mlp_hidden_dim, num_nodes * num_nodes)      # 输出维度N²
        )
    ...
    def forward(self, input, gate_out = False):
        ...
        n3 = torch.matmul(self.gate_network.We3, self.gate_network.memory)
        ...
        # Process the concatenated matrices through MLP
        g_processed_flat = self.gating_mlp(g_flat)  # (N*N,)
        #g_processed_flat = self.gating_mlp(g_concat)
        g_processed = g_processed_flat.view(self.num_nodes, self.num_nodes)

        
        # Apply softmax to ensure proper probability distribution
        g_processed = torch.softmax(g_processed, dim=-1)

        #g_processed = self.supports_dropout(g_processed)
        
        # Use the processed matrix as our support
        new_supports = [g_processed]
