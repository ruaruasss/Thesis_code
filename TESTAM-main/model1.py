...
adjdata_path = "/home/p_tf_zy/p_tf_01/TESTAM-main/data/METR-LA/adj_mx.pkl"  # 邻接矩阵路径

def load_adj_matrix(adjdata_path):
    """
    加载邻接矩阵文件

    参数:
        - adjdata_path: 邻接矩阵文件路径

    返回:
        - 处理后的邻接矩阵 (torch.Tensor)
    """
    try:
        with open(adjdata_path, "rb") as f:
            adj_data = pickle.load(f, encoding='latin1')
        
        if not isinstance(adj_data, (list, tuple)) or len(adj_data) < 3:
            raise ValueError("邻接矩阵数据格式错误，期望包含至少 3 个元素")

        adj_matrix = torch.tensor(adj_data[2], dtype=torch.float32)  # 提取邻接矩阵并转换为Tensor
        return adj_matrix
...
# Spatial Graph Construction Module
class SpatialGraphBuilder(nn.Module):
    """
    Constructs a graph structure based on spatial distance and node similarity.

    Args:
        - num_nodes: Number of nodes
        - hidden_size: Hidden layer size
        - sparsity: Percentage of edges to retain (20%, 40%, 60%, 80%)
        - init_graph: Initial graph structure (adjacency matrix)
    """
    def __init__(self, num_nodes, hidden_size, sparsity=0.7, init_graph=None):
        super(SpatialGraphBuilder, self).__init__()
        self.num_nodes = num_nodes
        self.sparsity = sparsity

        # If no initial graph is provided, create a zero matrix
        if init_graph is None:
            self.register_buffer('init_graph', torch.zeros(num_nodes, num_nodes))
        else:
            self.register_buffer('init_graph', init_graph)

        # MLP to compute node similarity
    ...
    def forward(self):
        ...
        node_embeddings = self.node_embeddings  # (N, hidden_size)

        # Use the initial graph as spatial distances
        spatial_distances = self.init_graph  # (N, N)

        # Compute pairwise node similarities
        node_i_emb = node_embeddings.unsqueeze(1)  # (N, 1, hidden_size)
        node_j_emb = node_embeddings.unsqueeze(0)  # (1, N, hidden_size)

        # Concatenate node embeddings and spatial distances
        pair_features = torch.cat([
            node_i_emb.expand(self.num_nodes, self.num_nodes, -1),  # (N, N, hidden_size)
            node_j_emb.expand(self.num_nodes, self.num_nodes, -1),  # (N, N, hidden_size)
            spatial_distances.unsqueeze(-1)  # (N, N, 1)
        ], dim=-1)  # (N, N, hidden_size * 2 + 1)

        # Compute similarity scores
        edge_scores = self.similarity_mlp(pair_features).squeeze(-1)  # (N, N)

        # Apply sparsity constraint, keeping only top-k% edges
        k = int(self.num_nodes * self.num_nodes * self.sparsity)
        if k > 0:
            # Flatten scores and select top k
            flat_scores = edge_scores.view(-1)
            _, top_indices = torch.topk(flat_scores, k)

            # Create a new sparse graph
            sparse_graph = torch.zeros_like(edge_scores)
            sparse_graph.view(-1)[top_indices] = flat_scores[top_indices]

            # Normalize the sparse graph
            row_sum = sparse_graph.sum(dim=1, keepdim=True)
            row_sum[row_sum == 0] = 1.0  # Avoid division by zero
            normalized_graph = sparse_graph / row_sum

            return normalized_graph
        else:
            # Return identity matrix if k=0
            return torch.eye(self.num_nodes, device=self.node_embeddings.device)


# Expert Model Based on Spatial Graph Construction
class SpatialExpert(nn.Module):
    """
    Expert model built on the graph structure constructed by SpatialGraphBuilder.
    """
    def __init__(self, hidden_size, num_nodes, dropout, layers, in_dim=2, out_dim=1, sparsity=0.7, init_graph=None, activation=nn.ReLU()):
        super(SpatialExpert, self).__init__()
        ...
        # Graph builder
        self.graph_builder = SpatialGraphBuilder(num_nodes, hidden_size, sparsity, init_graph)

        # Similar structure to STModel
        self.spatial = False
        s_gcn = gcn(c_in=hidden_size, c_out=hidden_size, dropout=dropout, supports_len=1, order=2)
        t_attn = QKVAttention(in_dim=hidden_size, hidden_size=hidden_size, dropout=dropout)
        ff = PositionwiseFeedForward(in_dim=hidden_size, hidden_size=4*hidden_size, dropout=dropout)
        norm = LayerNorm(normalized_shape=(hidden_size,))
    ...
    def forward(self, x, prev_hidden=None):
        ...
        supports = [graph]  # Use the constructed graph as support
...
class MemoryGate(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        B, N, T, _ = input.size()
        memories = self.query_mem(input)  # Query Memory Bank O_i^{(t)}
        scores = []
        for i, h in enumerate(hidden):
            hidden_att = attention(h,i) # z_e
            scores.append(self.sim(memories, hidden_att)) # r_e = g(z_e, O_i^(t))

        scores = torch.stack(scores, dim = -1)  # p_e = r_e / Σ r_e
    ...
    def attention(self, x, i):
        ...
        energy = torch.matmul(query, key.transpose(-1,-2))
        score = torch.softmax(energy, dim = -1)
        out = torch.matmul(score, value)  #ze
    ...
    def query_mem(self, input):
        B, N, T, _ = input.size()
        mem = self.memory
        query = torch.matmul(input, self.input_query)  # Q_i^{(t)} = X_i^{(t)} W_q
        energy = torch.matmul(query, mem.T) #  Q_i^{(t)} M[j]^⊤
        score = torch.softmax(energy, dim = -1) #  a_j
        out = torch.matmul(score, mem) # O_i^{(t)} = Σ a_j M[j]
        return out
...
class TESTAM(nn.Module):
    ...
    TESTAM model
    """
    def __init__(self, num_nodes, dropout=0.3, in_dim=2, out_dim = 1, hidden_size = 32, layers = 3, prob_mul = False, max_time_index = 288, adjdata=adjdata_path, device='cuda:0', **args):
        ...
        if adjdata:
            loaded_graph = load_adj_matrix(adjdata)
            if loaded_graph is not None:
                init_graph = loaded_graph.to(device)
        ...
        self.spatial_similarity_expert = SpatialExpert(hidden_size, num_nodes, dropout=dropout, layers=layers,in_dim=in_dim, out_dim=out_dim, init_graph=init_graph, sparsity=0.7)
        ...
        for model in [self.identity_expert, self.adaptive_expert, self.attention_expert, self.spatial_similarity_expert]:
        #for model in [self.spatial_similarity_expert]:
        #for model in [self.adaptive_expert, self.attention_expert, self.spatial_similarity_expert]:
    ...
    def forward(self, input, gate_out = False):
        ...
        # Add gradient clipping or normalization to prevent exploding gradients
        with torch.no_grad():
            for p in self.parameters():
        ...
        n2 = torch.matmul(self.gate_network.We2, self.gate_network.memory) # E = M W_E
        ...
        time_index = time_index.long()  # 将 time_index 转换为 LongTensor
        ...
        #_, o_spatial_sim, h_spatial_sim = self.spatial_similarity_expert(input, None)
        ...
        # 执行新增的空间相似度专家
        _, o_spatial_sim, h_spatial_sim = self.spatial_similarity_expert(input, h_future)

        # 将四个专家的输出堆叠
        #ind_out = torch.stack([o_identity, o_adaptive, o_spatial_sim], dim=-1)
        ind_out = torch.stack([o_identity, o_adaptive, o_attention, o_spatial_sim], dim=-1)
        #ind_out = torch.stack([o_adaptive, o_attention], dim = -1)
        #ind_out = torch.stack([o_spatial_sim], dim = -1)

        B, N, T, _ = o_identity.size()
        #B, N, T, _ = o_spatial_sim.size()
        #gate_in = [h_spatial_sim[-1]]
        #gate_in = [h_adaptive[-1], h_attention]
        gate_in = [h_identity[-1], h_adaptive[-1], h_attention, h_spatial_sim[-1]]
        ...
        outs = [o_identity, o_adaptive, o_attention, o_spatial_sim]
        counts = []
...
if __name__ == "__main__":
    n = 207
    model = TESTAM(num_nodes = n, in_dim = 3, out_dim = 2, supports = [torch.zeros(n,n).cuda() for _ in range(2)], adjdata=adjdata_path)
